...
def run():
    ...
    script_utils_obj.talk_to_user(
        'You can give the empty response too, in that case default options will be used.', '2.', 2)
    script_utils_obj.talk_to_user(
        '------------------------------------------------------------------------------------------')
    script_utils_obj.talk_to_user("")
    script_utils_obj.talk_to_user("")
    ...
    while True:
        ...
        try:
            ...
            source_dir_path = script_utils_obj.ask_question(
                "Enter the source directory name : ", '*', 1)

            if not source_dir_path:
                script_utils_obj.talk_to_user(
                    "Source directory path is mandatory, please provide an absolute path.", '', 4)
                continue

            file = pathlib.Path(source_dir_path)
            if not file.exists():
                script_utils_obj.talk_to_user(
                    "Source directory does not exist !!!", '', 4)
                continue

            cleaner.set__source_directory_name(source_dir_path)
            script_utils_obj.talk_to_user(
                "source directory is - '{}'".format(source_dir_path), '', 4)

            # accept the target directory path, to which the renamed files will be copied
            target_dir_path = script_utils_obj.ask_question(
                "Enter the target directory name [default is the 'target_dir' inside the source directory]:", '*', 1)

            if target_dir_path:
                file = pathlib.Path(target_dir_path)
                if not file.exists():
                    script_utils_obj.talk_to_user(
                        "Target directory does not exist. Creating the default target directory.", '', 4)
                    target_dir_path = os.path.join(
                        source_dir_path, 'target_dir')
                    directory = pathlib.Path(target_dir_path)
                    if not directory.exists():
                        os.mkdir(target_dir_path)
            else:
                script_utils_obj.talk_to_user(
                    "Empty input, creating the default target directory.", '', 4)
            ...
            script_utils_obj.talk_to_user(
                "target directory is - '{}'".format(target_dir_path), '', 4)

            # accept the input to decide if the file name should be in lower case or upper case or title case
            file_name_case = script_utils_obj.ask_question(
                "Enter the file name case [1 for lower | 2 for upper | 3 for title, 1 is the default value]: ", '*', 1)
            ...
            if file_name_case:
                try:
                    ...
                    if case_key in list(mapping.keys()):
                        ...
                        script_utils_obj.talk_to_user(
                            "Wrong input for file name case, using the default value", '', 4)
                ...
                script_utils_obj.talk_to_user(
                    "Empty input for file name case, using the default value", '', 4)

            script_utils_obj.talk_to_user(
                "File name case is - '{}'".format((mapping[case_key].name).title()), '', 4)

            # accept the file name seperator character
            file_name_separator = script_utils_obj.ask_question(
                "Enter the file name word separator ['_' or '-', ' ' is the default]: ", '*', 1)
            ...
            if file_name_separator:
                ...
                if file_name_separator in separators:
                    ...
                    script_utils_obj.talk_to_user(
                        "Wrong input for file name separator, using the default value", '', 4)
            else:
                file_name_separator = ' '
                script_utils_obj.talk_to_user(
            ...
            script_utils_obj.talk_to_user(
                "File name separator is - '{}'".format(file_name_separator), '', 4)

            # accept the file name seperator character
            file_name_prefix = script_utils_obj.ask_question(
            ...
            if file_name_prefix:
                ...
                script_utils_obj.talk_to_user(
                    "Empty input for file name prefix, not using the file prefix.", '', 4)

            script_utils_obj.talk_to_user(
                "File name prefix is - '{}'".format(file_name_prefix), '', 4)
            ...
            filter_file_path = script_utils_obj.ask_question(
                "Enter the filter words file path [Default is None]: ", '*', 1)
            ...
            if filter_file_path:
                ...
                if file.exists():
                    ...
                    script_utils_obj.talk_to_user(
                        "File does not exist, using the default option", '', 4)
            else:
                filter_file_path = None
                script_utils_obj.talk_to_user(
                    "Empty input, using the default option", '', 4)

            script_utils_obj.talk_to_user(
                "Filter file path is - '{}'".format(filter_file_path), '', 4)
            ...
            script_utils_obj.talk_to_user(
                "Exit command detected! Exiting the program.", '', 4)
            break
        except KeyboardInterrupt:
            script_utils_obj.talk_to_user(
                "Keyboard interrupt detected! Exiting the program.", '', 4)
            break
        except EOFError:
            script_utils_obj.talk_to_user(
                "Keyboard interrupt detected! Exiting the program.", '', 4)
